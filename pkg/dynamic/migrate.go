...
// 参考示例参见Example。
// 如果需要更复杂的条件处理，可以参考Example自行使用DBWithTable、StructZero、StructValue、SliceVariable进行组合。
package dynamic
...
func NewTable(table string, fields ...Field) *Table {
	...
	tab.structType = reflect.StructOf(structFields)
	return tab
}

// DBWithTable作为helper方法，在链式操作之前明确指定要操作的表名。
...
func (tab *Table) StructValue(values []any) any {
	...
	N := min(elem.NumField(), len(values))
	for i := 0; i < N; i++ {
		field := elem.Field(i)
